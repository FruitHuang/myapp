...
import { getLatinDailyModel } from "../graphql/queries.ts";
import { updateLatinDailyModel } from "../graphql/mutations.ts";
const client = generateClient();
export default function LatinDailyModelUpdateForm(props) {
  const {
    id: idProp,
    latinDailyModel: latinDailyModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = latinDailyModelRecord
      ? { ...initialValues, ...latinDailyModelRecord }
      : initialValues;
    setSentence(cleanValues.sentence);
    setHigPitch(cleanValues.higPitch);
    setLowPitch(cleanValues.lowPitch);
    setErrors({});
  };
  const [latinDailyModelRecord, setLatinDailyModelRecord] = React.useState(
    latinDailyModelModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getLatinDailyModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLatinDailyModel
        : latinDailyModelModelProp;
      setLatinDailyModelRecord(record);
    };
    queryData();
  }, [idProp, latinDailyModelModelProp]);
  React.useEffect(resetStateValues, [latinDailyModelRecord]);
  const validations = {
    sentence: [],
    higPitch: [],
    lowPitch: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          higPitch: higPitch ?? null,
          lowPitch: lowPitch ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateLatinDailyModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: latinDailyModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "LatinDailyModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Sentence"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || latinDailyModelModelProp)}
